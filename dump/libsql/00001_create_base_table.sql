...
CREATE UNIQUE INDEX IF NOT EXISTS confirmation_token_idx ON users (confirmation_token) WHERE confirmation_token GLOB '*[^0-9 ]*';
CREATE UNIQUE INDEX IF NOT EXISTS email_change_token_current_idx ON users (email_change_token_current) WHERE email_change_token_current GLOB '*[^0-9 ]*';
CREATE UNIQUE INDEX IF NOT EXISTS email_change_token_new_idx ON users (email_change_token_new) WHERE email_change_token_new GLOB '*[^0-9 ]*';
CREATE UNIQUE INDEX IF NOT EXISTS reauthentication_token_idx ON users (reauthentication_token) WHERE reauthentication_token GLOB '*[^0-9 ]*';
CREATE UNIQUE INDEX IF NOT EXISTS recovery_token_idx ON users (recovery_token) WHERE recovery_token GLOB '*[^0-9 ]*';
