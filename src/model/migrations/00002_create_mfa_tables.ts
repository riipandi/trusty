...
import * as tMfaAmrClaim from '@/model/schema/mfa_amr_claim.ts';
import * as tMfaChallenge from '@/model/schema/mfa_challenge.ts';
import * as tMfaFactor from '@/model/schema/mfa_factor.ts';
...
const MfaAmrClaimMigrationQuery = (db: KyselyDatabase) =>
  db.schema
    ...
    .addColumn('authentication_method', 'text', (col) =>
      col.notNull())
...
const MfaChallengeMigrationQuery = (db: KyselyDatabase) =>
  db.schema
    ...
    .addColumn('created_at', 'integer', (col) =>
      col.defaultTo(TIMESTAMP_MS).notNull())
    .ifNotExists();
...
const MfaFactorMigrationQuery = (db: KyselyDatabase) =>
  db.schema
    ...
    .$call(TIMESTAMPS_COLUMN)
    .addCheckConstraint(
      'mfa_factors_factor_type_check',
      sql`factor_type IN ('totp','webauthn')`,
    )
    .addCheckConstraint(
      'mfa_factors_factor_status_check',
      sql`status IN ('unverified','verified')`,
    )
    .ifNotExists();
