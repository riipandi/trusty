...
import { User } from '@/model/schema/user.ts';
...
export async function findUserById(
  ...
  const user = await db.selectFrom('users').where('id', '=', id).selectAll()
    .executeTakeFirst();
  return user ?? null;
}
...
export async function isExistsByEmail(
  db: KyselyDatabase,
  email: string,
): Promise<boolean> {
...
export async function isExistsById(
  db: KyselyDatabase,
  id: string,
): Promise<boolean> {
