...
import { Argon2id } from 'npm:oslo/password';

import { type KyselyDatabase } from '@/model/client.ts';
import { InsertUser } from '@/model/schema/user.ts';
  ...
  try {
    ...
    await db.transaction().execute(async (trx) => {
      ...
      const passwordPromises = newUsers.map(async ({ id: user_id, email }) => {
        ...
        if (!userWithPassword) {
          console.warn(
            `No password found for user with email '${email}'. Skipping.`,
          );
        ...
        return trx.insertInto('passwords').values({
          user_id,
          encrypted_password,
        }).execute();
      });
